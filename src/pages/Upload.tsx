  ...
  return (
    <div className="max-w-3xl mx-auto">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold text-foreground">AutoReq — Requirements Extraction Tool</h1>
        <p className="text-sm text-muted-foreground mt-1.5">
          Upload your stakeholder document to extract and validate software requirements.
        </p>
      </header>

      <div className="border border-border rounded bg-card p-6 space-y-5">
        <div>
          <label className="block text-xs font-medium mb-1.5">Document Name <span className="text-muted-foreground font-normal">(optional)</span></label>
          <input
            type="text"
            value={name}
        ...
        {!usePaste ? (
          <div>
            <label className="block text-xs font-medium mb-1.5">Document</label>
            <label
              htmlFor="file-input"
              className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded p-10 cursor-pointer hover:border-ring hover:bg-secondary/40 transition-colors"
            ...
            >
              {file ? (
                ...
                <>
                  ...
                  <span className="text-sm font-medium">Drop a PDF or DOCX here</span>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5">Or paste meeting notes / transcript directly</label>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              rows={10}
              className="w-full px-3 py-2 text-sm border border-input rounded bg-background focus:outline-none focus:ring-2 focus:ring-ring font-mono"
              ...
              {STAGES.map((s, i) => {
                ...
                return (
                  <div key={s} className="flex-1 flex items-center gap-2">
                    ...
                    {i < STAGES.length - 1 && <div className={`flex-1 h-px ${done ? "bg-primary" : "bg-border"}`} />}
                  </div>
                );
              })}
            </div>
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            className="bg-primary text-primary-foreground px-5 py-2 rounded text-sm font-medium hover:bg-primary/90 disabled:opacity-50"
          >
